...
void generate_host(Engine &ctx, CountT k_ary)
{
    ...
    for (uint32_t i = 0; i < num_host; i++) {
        ...
        ctx.get<PktBuf>(sender_e).cur_num = 0;
        ...
        uint32_t ett_idx = ctx.data().numSender;
        ctx.get<NextHop>(sender_e).next_hop = next_hop_table_host_to_sw[ett_idx];

        ctx.get<SimTime>(sender_e).sim_time = 0;
        ctx.get<SimTimePerUpdate>(sender_e).sim_time_per_update = LOOKAHEAD_TIME; //1000ns
        ...
        int64_t flow_size = 2000*1000; // 10 * 1000*10*1460; // 20000;// ~10ms
        // if (i >= 2) {
        ...
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dst = i+num_host/2;  // i+2; // 
        ...
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].pfc_state = PFCState::RESUME;

        ctx.get<SendFlowQueue>(sender_e).send_flow[0].last_ack_timestamp = 0; //1ms
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].nxt_pkt_event = 0;

        ctx.get<SendFlowQueue>(sender_e).send_flow[0].m_rate = DCQCN_CurrentRate;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].tar_rate = DCQCN_TargetRate;

        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_Alpha = DCQCN_Alpha;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_G = DCQCN_G;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_AIRate = DCQCN_AIRate;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_HAIRate = DCQCN_HAIRate;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_RateDecreaseInterval = DCQCN_RateDecreaseInterval;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_RateIncreaseInterval = DCQCN_RateIncreaseInterval;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_AlphaUpdateInterval = DCQCN_AlphaUpdateInterval;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_IncreaseStageCount = DCQCN_IncreaseStageCount;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_RateDecreaseNextTime = DCQCN_RateDecreaseNextTime;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_RateIncreaseNextTime = DCQCN_RateIncreaseNextTime;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_AlphaUpdateNextTime = DCQCN_AlphaUpdateNextTime;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].dcqcn_RateIsDecreased = DCQCN_RateIsDecreased;
        ctx.get<SendFlowQueue>(sender_e).send_flow[0].CNPState = false;
    ...
    for (uint32_t i = 0; i < num_host; i++) {
        ...
        ctx.get<PktBuf>(receiver_e).cur_num = 0;
        ...
        uint32_t ett_idx = ctx.data().numReceiver;
        ctx.get<NextHop>(receiver_e).next_hop = next_hop_table_host_to_sw[ett_idx];

        ctx.get<SimTime>(receiver_e).sim_time = 0;
        ctx.get<SimTimePerUpdate>(receiver_e).sim_time_per_update = LOOKAHEAD_TIME; //1000ns
        ...
        ctx.get<RecvFlowQueue>(receiver_e).recv_flow[0].src = i-2; //i - num_host/2; // i-2;  //i-1;
        ...
        ctx.get<RecvFlowQueue>(receiver_e).recv_flow[0].last_cnp_timestamp = 0;
    }
