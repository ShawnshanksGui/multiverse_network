...
void generate_in_port(Engine &ctx, CountT k_ary)
{
    ...
    for (int32_t i = 0; i < num_edge_switch; i++) {
        for (int32_t j = 0; j < k_ary; j++) {
            ...
            ctx.get<GlobalPortID>(in_port).global_port_id = ctx.data().numInPort + i*k_ary+j;
...
void generate_e_port(Engine &ctx, CountT k_ary)
{
    ...
    for (int32_t i = 0; i < num_edge_switch; i++) {
        for (int32_t j = 0; j < k_ary; j++) {
            ...
            ctx.get<GlobalPortID>(e_port).global_port_id = ctx.data().numEPort + i*k_ary+j; //i*k_ary+j;
    ...
    for (int32_t i = 0; i < num_aggr_switch; i++) {
        for (int32_t j = 0; j < k_ary; j++) {
            ...
            ctx.get<GlobalPortID>(e_port).global_port_id = ctx.data().numEPort + i*k_ary+j;
