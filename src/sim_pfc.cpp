...
inline void _enqueue(T1 &queue, T2 packet) {
    //queue.pkts[queue.tail] = {packet.src_id, packet.dst_id, packet.pkt_size, packet.flow_id, packet.enqueue_time, packet.dequeue_time};
...
inline void send(Engine &ctx, SenderID &_sender_id, NextHop &_next_hop, \
    ...
    while (_sim_time.sim_time < end_time) {
        ...
        int8_t flow_priority = 0;
        PacketType packet_type = PacketType::DATA;
        Pkt packet = {_sender_id.sender_id, receiver_id, pkt_size, flow_id, enqueue_time, dequeue_time, flow_priority, packet_type};
...
{   
    if (_global_port_id.global_port_id == 0) {
        printf("*******Enter into set_forward_plan sys*********\n");
    }
    //reset
    ...
    while(true) {
        ...
        if (packet.packet_type == PacketType::DATA || packet.packet_type == PacketType::ACK) {
            int32_t sw_idx = _switch_id.switch_id;
            //printf("sw_idx: %d, ", sw_idx);
            Entity sw_id = ctx.data()._switch[sw_idx];
            int32_t next_hop_link_idx = ctx.get<FIBTable>(sw_id).fib_table[packet.dst_id][packet.flow_id];
        ...
        //  broadcast PFC pause/resume frame into all egress port excepting for the current port itself
            ...
            if (packet.dst_id == 0) {// the pfc frame is received from the downstream egress port 
                Entity e_port = ctx.data().ePorts[_global_port_id.global_port_id]; 
                // the idx of priority queue is placed in the filed of packet.dst_id
                // the PFC state is placed in the filed of the packet.src_id
                if (packet.packet_type == PacketType::PFC_PAUSE)
                    ctx.get<PktQueue>(e_port).queue_pfc_state[packet.dst_id] = PFCState::PAUSE;
                else ctx.get<PktQueue>(e_port).queue_pfc_state[packet.dst_id] = PFCState::RESUME;
            }

            for (int i = 0; i < INPORT_NUM; i++) { // broadcast
                if (i == _local_port_id.local_port_id) continue;
    ...
    for(CountT i = _switch_id.switch_id*K_ARY; i < (_switch_id.switch_id+1)*K_ARY; i++) { 
        ...
        while(count_bit--) {
            if ((fwd_bitmap & (1<<num)) != 0) { // the packet should be forward to the e_port
                ...
                uint8_t queue_idx = packet.flow_priority;
                _enqueue(_queue.pkt_buf[queue_idx], packet);
                num += 1;
            }
              ...
              SwitchID &_switch_id)
{
    ...
    uint8_t queue_num = ctx.get<QueueNumPerPort>(ctx.data()._switch[_switch_id.switch_id]).queue_num_per_port;
    // strict priority(SP) to schedule packets 
    for (uint8_t i = 0; i < queue_num; i++) {
        if (get_queue_len(_pkt_queue.pkt_buf[i]) <= 0) continue;
        
        if (_pkt_queue.queue_pfc_state[i] == PFCState::PAUSE) continue;
        ...
        while(true) {
            ...
            // fetch packet from buffer, while checking whether buffer is empty
            Pkt packet;
            if(!_dequeue(_pkt_queue.pkt_buf[i], packet)) break; // dequeue a packet and remove it
            if(packet.enqueue_time >= end_time) break;
            ...
            if (inst_queue_len >= BUF_SIZE*(PFC_THR/100.0)) {
                int32_t PFC_FOR_UPSTREAM = 1;
                int64_t pfc_enqueue_time = packet.enqueue_time;
                int16_t queue_idx = packet.flow_priority; // the idx of priority queue is placed in the filed(4-th element) of packet.dst_id
                int8_t flow_priority = 0; // and the pfc frame is set with highest flow priority
                Pkt pfc_frame = {0, PFC_FOR_UPSTREAM, 64, queue_idx, pfc_enqueue_time, 0, flow_priority, PacketType::PFC_PAUSE}; // and PacketType is PFC PAUSE
...
void Sim::setupTasks(TaskGraphBuilder &builder, const Config &)
{
    ...
    auto forward_sys = builder.addToGraph<ParallelForNode<Engine, _forward, SchedTrajType, \
                                          LocalPortID, GlobalPortID, PktQueue, SwitchID, SimTime, SimTimePerUpdate>>({set_forward_plan_sys});

    auto remove_pkts_sys = builder.addToGraph<ParallelForNode<Engine, remove_pkts, \
                                              LocalPortID, GlobalPortID, PktBuf, ForwardPlan, SimTime, SimTimePerUpdate>>({forward_sys}); 
    ...
    auto transmit_sys = builder.addToGraph<ParallelForNode<Engine, transmit, SchedTrajType, \
                                           PortType, GlobalPortID, NextHop, NextHopType, PktQueue, TXHistory, \
                                           SSLinkDelay, LinkRate, SimTime, SimTimePerUpdate, SwitchID>>({remove_pkts_sys}); 
