...
inline void comm_set_flow(Engine &ctx, NPU_ID _npu_id,
    ...
    if(_sim_time.sim_time >= 1000) {return;}
    ...
    if (_npu_id.npu_id >= npu_num-2) return;
    printf("npu_num: %d, npu_id: %d\n", npu_num, _npu_id.npu_id);

    uint32_t src = _npu_id.npu_id;
    uint32_t dst = _npu_id.npu_id+1;

    int64_t flow_size = 2000*10000;

    FlowEvent flow_event = {0, src, dst, 1, 0, flow_size, _sim_time.sim_time+1000, 0, FlowState::UNCOMPLETE};
